        ...
        >
          ...
          >
            ...
            >
              Resources
              <ChevronDown
                className={`w-4 h-4 transition-transform duration-200 ${isResourcesOpen ? "rotate-180 text-zinc-900" : ""}`}
              />
            </button>
            ...
            <AnimatePresence>
              {isResourcesOpen && (
                <motion.div
                  ...
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-80 bg-white border border-zinc-200 rounded-2xl shadow-xl p-2 overflow-hidden"
                >
                  <div className="flex flex-col gap-0.5">
                    {resources.map((item) => (
                      ...
                      >
                        <span className="text-sm font-semibold text-zinc-900 group-hover:text-teal-600 transition-colors block">
                          {item.name}
                        </span>
                        <span className="text-xs text-zinc-500 block leading-relaxed mt-0.5">
                          {item.description}
                        </span>
                      </Link>
                    ))}
                ...
                <AnimatePresence>
                  {isMobileResourcesOpen && (
                    <motion.div
                      ...
                      className="overflow-hidden mt-3 flex flex-col gap-3 py-2 text-center"
                    >
                      {resources.map((item) => (
                        <Link
                          key={item.name}
